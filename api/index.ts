...
app.post("/api/purchases", authenticate, async (req: any, res) => {
  try {
    const { itemId, quantity, pricePerUnit, supplier, source, date } = req.body;
    const totalAmount = quantity * pricePerUnit;
    const purchaseDate = date ? new Date(date) : new Date();

    // 1. Record Purchase
    const [result]: any = await pool.query(
      "INSERT INTO purchases (itemId, quantity, pricePerUnit, totalAmount, supplier, source, date, userId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [itemId, quantity, pricePerUnit, totalAmount, supplier, source, purchaseDate, req.user.id]
    );
    ...
    // 3. Adjust Balance (Add as Approved Expense)
    ...
    await pool.query(
      ...
      [req.user.id, totalAmount, "Stock Purchase", itemName, source, `Purchase of ${quantity} ${itemName} from ${supplier || 'Unknown'}`, purchaseDate]
    );
